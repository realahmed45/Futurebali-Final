    ...
    <div className="packages-container">
      ...
      <div className="packages-header">
        <div className="packages-header-content">
          ...
          <p className="packages-description">
            <Link to="/" className="breadcrumb-link">Home</Link> &gt; Case Study
          </p>
        </div>
      </div>
      ...
      <div style={{ textAlign: 'center', padding: '20px 0' }}>
        <p style={{ fontSize: '18px', color: '#555' }}>
          Explore case studies across various categories to see our exceptional work.
        </p>
      <div className="category-selection">
        ...
        <p 
          ...
          className={`category-link ${category === 'Bedroom' ? 'selected' : ''}`}
        >Bedroom</p>
        <p 
          onClick={() => handleCategoryChange('Garden')} 
          className={`category-link ${category === 'Garden' ? 'selected' : ''}`}
        >Garden</p>
        <p 
          onClick={() => handleCategoryChange('Kitchen')} 
          className={`category-link ${category === 'Kitchen' ? 'selected' : ''}`}
        >Kitchen</p>
        <p 
      ...
      <div className="gallery-section">
        ...
        <div className="image-grid">
          {imagePaths[category].map((image, index) => (
            <div
              key={index}
              className={`image-container ${isSlidingOut ? 'slide-out' : 'slide-in'}`}
